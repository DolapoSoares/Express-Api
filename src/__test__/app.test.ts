...
import app from '../app'
import { deleteAuthor } from '../controller/authorsController'
...
describe('GET AUTHORS', () => {
    ...
    test('should return 200 status for a single author', async () => {
        const res = await (await supertest(app).get('/authors/1'))
        expect(res.statusCode).toEqual(200);
    })
})

describe('GET Books', () => {
    test('should return 200 status for a single book', async () => {
        const res = await (await supertest(app).get('/authors/1/books/book1'))
        expect(res.statusCode).toEqual(200);
    })
})
...
describe('POST BOOK', () => {
    // test('return status code 201 if author data is passed correctly ', async () => {
    //     await supertest(app).post('/authors/1/books').send({
    ...
    // })
    test('should return bad request if some data is missing', async()=>{
        const res = await supertest(app).post('/authors/1/books').send({
           //serialNumber is missing in the data below
            books: [
                {
                    "name": "Tomorrow is coming",
                    "isPublished": true,
...
describe('DELETE AN AUTHOR', ()=> {
    ...
    expect(removedAuthor.body.message).toEqual(`author ${newAuthor.body.data.id} deleted`)
    })
})
describe('PUT AUTHOR', ()=>{
    test('it responds with an updated data', async ()=> {
        ...
        .post('/authors')
        .send( {
            "author": "emmanuel adeshokan",
            "age": 25,
            "address": "5, Wall Street, Oak street, idumota lagos ",
            "books": [
                {
                    "name": "Tomorrow is coming",
                    "isPublished": true,
